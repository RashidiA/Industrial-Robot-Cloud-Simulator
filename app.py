...
ROBOT_REGISTRY = {
    "ABB_6700": {
        "links": [
            {"name": "A1", "trans": [0, 0, 0.78],   "rot": [0, 0, 1]},
            {"name": "A2", "trans": [0.32, 0, 0],    "rot": [0, 1, 0]},
            {"name": "A3", "trans": [0, 0, 1.28],    "rot": [0, 1, 0]},
            {"name": "A4", "trans": [1.142, 0, 0.2], "rot": [1, 0, 0]},
            {"name": "A5", "trans": [0.2, 0, 0],     "rot": [0, 1, 0]},
            {"name": "A6", "trans": [0.2, 0, 0],     "rot": [1, 0, 0]},
        ],
        "fallback_heights": [0.78, 0.5, 1.28, 0.4, 0.2, 0.2, 0.1]
    },
    "ABB_6600": {
        "links": [
            {"name": "A1", "trans": [0, 0, 0.70],   "rot": [0, 0, 1]},
            {"name": "A2", "trans": [0.30, 0, 0],    "rot": [0, 1, 0]},
            {"name": "A3", "trans": [0, 0, 1.15],    "rot": [0, 1, 0]},
            {"name": "A4", "trans": [1.050, 0, 0.2], "rot": [1, 0, 0]},
            {"name": "A5", "trans": [0.2, 0, 0],     "rot": [0, 1, 0]},
            {"name": "A6", "trans": [0.2, 0, 0],     "rot": [1, 0, 0]},
        ],
        "fallback_heights": [0.70, 0.45, 1.15, 0.35, 0.2, 0.2, 0.1]
    },
    "ABB_4400": {
        "links": [
            {"name": "A1", "trans": [0, 0, 0.68],   "rot": [0, 0, 1]},
            {"name": "A2", "trans": [0.20, 0, 0],    "rot": [0, 1, 0]},
            {"name": "A3", "trans": [0, 0, 0.88],    "rot": [0, 1, 0]},
            {"name": "A4", "trans": [0.780, 0, 0.15], "rot": [1, 0, 0]},
            {"name": "A5", "trans": [0.15, 0, 0],    "rot": [0, 1, 0]},
            {"name": "A6", "trans": [0.15, 0, 0],    "rot": [1, 0, 0]},
        ],
        "fallback_heights": [0.68, 0.35, 0.88, 0.30, 0.15, 0.15, 0.1]
    },
    "KUKA_KR150": {
        "links": [
            {"name": "A1", "trans": [0, 0, 0.75],   "rot": [0, 0, 1]},
            {"name": "A2", "trans": [0.35, 0, 0],    "rot": [0, 1, 0]},
            {"name": "A3", "trans": [0, 0, 1.35],    "rot": [0, 1, 0]},
            {"name": "A4", "trans": [1.200, 0, 0.25], "rot": [1, 0, 0]},
            {"name": "A5", "trans": [0.25, 0, 0],    "rot": [0, 1, 0]},
            {"name": "A6", "trans": [0.21, 0, 0],    "rot": [1, 0, 0]},
        ],
        "fallback_heights": [0.75, 0.55, 1.35, 0.45, 0.25, 0.21, 0.1]
    }
}

# --- 3. OPERATOR INTERFACE SIDEBAR PROFILE SELECTION ---
with st.sidebar:
    st.title("📟 Teach Pendant Pro")
    
    with st.expander("🛠️ Layout Setup", expanded=True):
        # Scan folder names from directory matching your layout image structure
        ...
        if os.path.exists(robot_folder_path):
            ...
            if scanned_folders:
                available_profiles = scanned_folders

        # DYNAMIC SELECTION DROP-DOWN
...
# --- 4. DYNAMIC KINEMATICS ENGINE CONTEXT ---
@st.cache_resource
def build_dynamic_chain(profile_name):
    config = ROBOT_REGISTRY.get(profile_name, ROBOT_REGISTRY["ABB_6700"])
    chain_links = [OriginLink()]
    for link in config["links"]:
        chain_links.append(URDFLink(
            name=link["name"], 
            origin_translation=link["trans"], 
            origin_orientation=[0, 0, 0], 
            rotation=link["rot"]
        ))
    return Chain(name=profile_name, links=chain_links, active_links_mask=[False, True, True, True, True, True, True])

robot_chain = build_dynamic_chain(selected_profile)
...
# --- 6. VIEWPORT CONTAINER COMPONENT ---
def build_embedded_viewport(payload):
    ...
    <body>
        ...
        <script>
            ...
            // Dynamic generation using structural profiles passed from the python server
            for(let i=0; i<7; i++) {
                let mesh;
            ...
            if(data.gunData && data.gunData.length > 0) {
                ...
                gunInternalMesh.scale.set(0.001, 0.001, 0.001); gunInternalMesh.rotation.y = data.gunRotZ; 
            ...
            if(data.jigData && data.jigData.length > 0) {
                ...
                m.scale.set(data.jigScale, data.jigScale, data.jigScale); m.rotation.x = data.rotX; m.rotation.y = data.rotY;
            ...
            function computeForwardKinematics(angles) {
                ...
                // Multi-Robot Transformation Pipeline
                for(let i=0; i<6; i++) {
            ...
            function updateSceneTransforms(transforms, gunOffset, jigX, jigY, jigZ, e1RotAngle) {
                ...
                if(links[6]) {
                    links[6].updateMatrixWorld(); gunMesh.position.copy(links[6].position);
                    gunMesh.quaternion.copy(links[6].quaternion); gunMesh.translateX(gunOffset);
                }
            ...
            for(let i=1; i<=6; i++) {
                const row = document.createElement('div'); row.className = 'jog-row';
            ...
            const e1Row = document.createElement('div'); e1Row.className = 'jog-row'; e1Row.style.marginTop = '10px'; e1Row.style.borderTop = '1px solid #333'; e1Row.style.paddingTop = '8px';
            e1Row.innerHTML = `<button class="jog-btn" id="btn-m-7">-</button><div class="jog-label" style="color:#ff9800;">E1</div><div class="val-display" id="val-7">${(localJointAngles[7] * (180 / Math.PI)).toFixed(1)}°</div><button class="jog-btn" id="btn-p-7">+</button>`;
            ...
            document.getElementById('btn-run-sim').addEventListener('click', () => {
                if(embeddedTrajectory.length < 2) { alert("Record at least 2 steps."); return; }
            ...
            function animate() {
                ...
                if (runSimulation && embeddedTrajectory.length >= 2) {
                    ...
                    interpolationFraction += (parseFloat(document.getElementById('sld-speed').value) / 100) * 0.04;
...
# Read binary meshes explicitly out of selected profiles folder
link_b64s = []
for i in range(7):
    ...
    # Fallback to general meshes asset structural base if specific folder asset is missing
...
scene_payload = {
    ...
    "jigX": jx_pos, "jigY": jy_pos, "jigZ": jz_pos,
    "rotX": float(j_rot_x) * (np.pi / 180.0), "rotY": float(j_rot_y) * (np.pi / 180.0),
    "jigScale": js_scale
}
