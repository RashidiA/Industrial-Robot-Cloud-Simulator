...
st.set_page_config(page_title="ABB 6700 OLP Pro-Simulator", layout="wide")
...
@st.cache_resource
def build_abb_chain():
    return Chain(name='ABB_6700', links=[
        OriginLink(),
        URDFLink(name="A1", origin_translation=[0, 0, 0.78],   origin_orientation=[0, 0, 0], rotation=[0, 0, 1]),
        URDFLink(name="A2", origin_translation=[0.32, 0, 0],    origin_orientation=[0, 0, 0], rotation=[0, 1, 0]),
        URDFLink(name="A3", origin_translation=[0, 0, 1.28],    origin_orientation=[0, 0, 0], rotation=[0, 1, 0]),
        URDFLink(name="A4", origin_translation=[1.142, 0, 0.2], origin_orientation=[0, 0, 0], rotation=[1, 0, 0]),
        URDFLink(name="A5", origin_translation=[0.2, 0, 0],     origin_orientation=[0, 0, 0], rotation=[0, 1, 0]),
        URDFLink(name="A6", origin_translation=[0.2, 0, 0],     origin_orientation=[1, 0, 0], rotation=[1, 0, 0]),
    ], active_links_mask=[False, True, True, True, True, True, True])

robot_chain = build_abb_chain()
...
# --- 3. HYBRID EVENT ROUTER (SYNC WEBGL TRAJECTORY STORAGE & APP STATE) ---
...
def build_embedded_viewport(payload):
    ...
    <body>
        ...
        <script>
            ...
            const fallbackHeights = [0.78, 0.5, 1.28, 0.4, 0.2, 0.2, 0.1];
            ...
            function computeForwardKinematics(angles) {
                ...
                let m1 = new THREE.Matrix4().makeTranslation(0, 0, 0.78);
                ...
                let m2 = new THREE.Matrix4().makeTranslation(0.32, 0, 0);
                ...
                let m3 = new THREE.Matrix4().makeTranslation(0, 0, 1.28);
                ...
                let m4 = new THREE.Matrix4().makeTranslation(1.142, 0, 0.2);
                ...
                let m5 = new THREE.Matrix4().makeTranslation(0.2, 0, 0);
                ...
                let m6 = new THREE.Matrix4().makeTranslation(0.2, 0, 0);
...
for i in range(7):
    ...
    link_b64s.append(get_file_base64_cached(os.path.join(BASE_DIR, "assets", "meshes", name), get_file_hash(os.path.join(BASE_DIR, "assets", "meshes", name))))
